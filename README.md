...
- 练习类型：支持笔画练习和汉字默写练习；默写练习只显示拼音与声调，不显示田字格底字。
- 笔顺动画：可查看单字或词语中多个汉字的笔顺，词语模式下可点选不同汉字切换播放。
- 拼音与朗读：使用浏览器语音能力辅助识字。
- iPad 书写体验：横屏下练习和反馈区域尽量保持在一屏内，田字格尺寸更适合低龄儿童持笔书写，并包含基础防手掌误触逻辑。
- 逐笔校验：结合笔画类型、顺序和笔画位置做反馈，减少同类笔画调换顺序仍被判对的问题。
- 练习库与记录：使用 IndexedDB 在本机保存设置、练习记录、笔画练习库、默写练习库和自定义字词库。
- 自定义字词库：支持输入单字或词语，编辑时可用空格、逗号、顿号或分号分隔。
...
http://127.0.0.1:4173/index.html?view=practice&chars=学习&practiceType=dictation
